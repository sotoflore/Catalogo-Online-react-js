...
const CardProduct = ({ product, onShowDetails, addToCart }) => {
  ...
  return (
    <Card className='h-100 shadow p-1 rounded-3 border-0 bg-card'>
      <Card.Img variant="top" src={product.image} className='img-fluid rounded-2' alt={product.name} />
      <Card.Body>
        <Card.Title className='fs-6 fw-bold text-white'>{product.name}</Card.Title>
        <p className='badge text-bg-primary'><small>{product.category}</small></p>
        <p className='text-white fw-bold text-small'>Precio: <span className='badge text-bg-danger p-2'>S/{product.price}.00</span></p>
